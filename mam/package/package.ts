		...
		@$mol_mem
			...
			if (dir.exists()) {
				try {
					...
					for (let repo_node of mapping.select('pack', dir.name(), 'git').kids) {
						const repo = repo_node.kids[0]?.value
						if (!repo) continue
						this.$.$mol_exec(dir.path(), 'git', 'init')

						const res = this.$.$mol_exec(dir.path(), 'git', 'remote', 'show', repo)
			...
			for (let repo_node of mapping.select('pack', dir.name(), 'git').kids) {
				const repo = repo_node.kids[0]?.value
				if (!repo) continue
				this.$.$mol_exec(root.dir().path(), 'git', 'clone', '--depth', '1', repo, dir.relate(root.dir()))
				dir.reset()
				return true
			}
